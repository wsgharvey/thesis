...
\end{equation}
where $\Omega(\rvx_0) = \rvy$ and $\Omega(\hat{\rvx}_\theta(\mX_t, t))$ are the predicted values of $\rvx_0$ corresponding to observed components. \Cref{eq:tddm-guidance-term} can be computed by differentiating through the score network $\rvs_\theta$. Including the guidance weight $w$ introduced in \cref{sec:reconstruction-guidance}, which we set to $w = 1$ in our experiments, our final score function estimate is
...
% We use guidance weight $w = 1$.

% To do so we will make changes in our estimated score function so that it approximates $\nabla_{\rvx_t} q_{t|0} (\rvx_t | \Omega(\mX_0))$, where $\Omega(\mX_0)$ refers to the observed 
