...
To perform reconstruction guidance, we will assume that we know a correspondence between components of $\rvy$ and components of $\rvx_t$. For molecular data, where the set of components is permutation invariant, we can simply match the first $n_\rvy$ components in $\rvx_t$ to the $n_\rvy$ components in $\rvy$. In the video case, we will condition on the first and last frame so match the first and second frame in $\rvy$ to the first and last frames of $\rvx_t$ respectively. We will implicitly condition on this correspondence whenever we condition on $\rvy$.
...
\end{equation}
\Cref{eq:tddm-guidance-term} can be computed by differentiating through the score network $\rvs_\theta$. Including the guidance weight $w$ introduced in \cref{sec:reconstruction-guidance}, which we set to $w = 1$ in our experiments, our final score function estimate is
