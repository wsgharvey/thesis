...
\end{align}
Introducing the analytic score of the Gaussian $\rvx_0$, and defining our implicit score-matching loss as $\mathcal{L}_\text{ISM}(\theta, t) := \mathcal{L}_\text{SM}(\theta, t) - C_2$, we get
\begin{align}
...
% Consider the joint distribution $q_{\sigma(t)}(\rvx_t, \rvx_0, \rvy)$ in which we explicitly consider the relationship between noised data $\rvx_t$ and clean data $\rvx_0$. Its conditional $q_{\sigma(t)}(\rvx_t|\rvx_0,\rvy)$ is Gaussian with mean $\rvx_0$ and variance $\sigma(t)^2$, following \cref{eq:diffusion-xt-given-x0}. Its score function can therefore be analytically computed as $\frac{\rvx_0 - \rvx_t}{\sigma(t)^2}$. Following \citet{vincent2011connection}, we now show that fitting $\rvs_\theta(\rvx, \rvy, \sigma)$ to this quantity with a mean-squared error loss is equivalent to fitting it to score of the distribution of interest, $q_{\sigma(t)}(\rvx_t | \rvy)$.
