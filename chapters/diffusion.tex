...
% As discussed in \cref{eq:diffusion-equivalent-parameterisations}, the diffusion model can be trained to predict any of a selection of different quantities and then, at test-time, any of these can be used to estimate the desired score function. Recall that we described predicting the reparameterisation noise $\epsilon$, clean data $\rvx$, or the score $\nabla_{\rvx_\sigma} q(\rvx_\sigma|\rvy)$. All of these techniques have been used in the literature~\citep{ho2020denoising,karras2022elucidating,song2020score}. 
...
\begin{equation}
    \mathrm{d}\rvx = g(t) \mathrm{d}\rvw
\end{equation}
by including the drift term $b_t: \mathbb{R}^n \rightarrow \mathbb{R}^n$, which in general can depend on the value of the current state $\rvx$:
\begin{align} \label{eq:forward-diffusion-with-drift}
    \mathrm{d}\rvx = b(t) \rvx \mathrm{d}t + g(t) \mathrm{d}\rvw.
\end{align}
If $b(t)$ is defined to be zero for all $t$, this diffusion process is equivalent to that described previously in this chapter. If $b(t)$ is chosen to be negative, there is a negative feedback loop which prevents the value of $\rvx$ from growing unboundedly with $t$. In particular, a common choice is to use $b(t) := - \frac{1}{2}g(t)^2$ which results in what is known as a ``variance-preserving'' diffusion process. The reason for this is that if we assume that $\rvx_0$ (i.e. the data distribution) has unit variance, using $b(t) := - \frac{1}{2}g(t)^2$ will cause all marginals $q(\rvx_t)$ to similarly have unit variance. See Appendix B of \citet{song2020score} for a proof. Correspondingly, the process that we have described with $b(t) := 0$ is sometimes called a ``variance-exploding'' diffusion process. The variance of $q(t)$ is simply the sum of the variance of the data distribution and the variance of the added noise $\sigma_t^2 = \int_0^t g(t') \mathrm{d}t'$, and so ``explodes'' as the variance of the added noise grows as $t \rightarrow \infty$.
